...
class TdClient(
    ...
    var onUpdate: ((org.drinkless.tdlib.TdApi.Object) -> Unit)? = null
    ...
    fun send(
        query: org.drinkless.tdlib.TdApi.Function,
        cb: (org.drinkless.tdlib.TdApi.Object) -> Unit = {}
    ) {
        client.send(query, Client.ResultHandler { obj -> cb(obj) })
    }

    fun setTdlibParameters(databaseDir: String, filesDir: String) {
        val req = org.drinkless.tdlib.TdApi.SetTdlibParameters(
            false,
            databaseDir,
            filesDir,
            ByteArray(0),
